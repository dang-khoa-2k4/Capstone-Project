\subsection{BezierGCS: Hoạch định quỹ đạo với đường cong Bézier}
\label{subsec:beziergcs}
...
định nghĩa khung \emph{BezierGCS} cho bài toán hoạch định chuyển động liên tục.
...
toán về dạng MICP khả giải. Hình~\ref{fig:beziergcs_pipeline} tóm tắt luồng xử
lý tổng thể.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{experiments/image/beziergcs_pipeline.jpg}
    \caption[Motion planning bằng BezierGCS trên đồ thị các tập lồi]{%
    ...
    \textbf{(B)} Không gian tự do $\mathcal{C}_{\mathrm{free}}$ được phân hoạch
    thành các vùng an toàn lồi $\{Q_i\}$, ví dụ bằng IRIS, ACD hoặc VCC.
...
\end{equation}
trong đó $\mathbf{r}_{v,k} \in \mathbb{R}^{n_c}$ là các \emph{điểm điều khiển
    không gian} và $B_{k,d}(s) = \binom{d}{k} s^k (1-s)^{d-k}$ là đa thức Bernstein
...
\subsubsection{Bài toán BezierGCS hoàn chỉnh}
\label{subsubsec:beziergcs_complete}

Kết hợp cấu trúc luồng mạng từ~\eqref{eq:micp_formulation}, tham số hóa Bézier,
và các ràng buộc liên tục trên đây, bài toán BezierGCS đầy đủ được phát biểu
...
Sau khi áp dụng nới lỏng phối cảnh toàn bộ, bài toán~\eqref{eq:beziergcs_full}
trở thành Quy hoạch Nón Bậc hai Nguyên Hỗn hợp (MI-SOCP), có thể giải bởi các
bộ giải SOCP/MISOCP thương mại (Gurobi, Mosek) hoặc mã nguồn mở (CLARABEL,
ECOS) với bảo đảm tính tối ưu toàn cục trong giới hạn dung sai nguyên.

\subsubsection{Ví dụ minh họa: BezierGCS bậc hai trong không gian 2D}
\label{subsubsec:beziergcs_example2d}

Để minh họa khung BezierGCS, xét bài toán tìm đường đi hình học cho robot điểm
...
\begin{equation}
    ...
    \right).
\end{equation}
Khi đó bài toán BezierGCS bậc hai có thể viết gọn dưới dạng
\begin{equation}
    \begin{aligned}
