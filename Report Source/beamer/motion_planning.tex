...
\begin{frame}{TN2 - Mê cung $25\!\times\!25$: Linear vs Bézier GCS}
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.55\textwidth}
            ...
            \begin{itemize}
                \item \textbf{Linear GCS:} Bézier bậc $d=1$, $\dot{q}\in[-1,1]$.
            ...
            \vspace{0.3em}
            \[
                \delta_{\mathrm{opt}} := \frac{C_{\mathrm{round}} - C_{\mathrm{opt}}}{C_{\mathrm{opt}}}
            \]
            \textbf{Tối ưu toàn cục:} $y_e^*\!\in\!\{0,1\}$ tự động; $\delta_{\mathrm{opt}}=0\%$ nghĩa là cost trùng cận dưới, không cần rounding.
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{TN4 - Biểu đồ vận tốc: tự động giảm tốc tại nút giao và hành lang hẹp}
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.35\textwidth}
            \footnotesize
            Mô hình động học có thông số tốc độ dao động $0.6$-$0.75$ m/s, \textbf{tự động giảm tốc} tại:
            \begin{itemize}\itemsep0pt
            ...
            \vspace{0.4em}
            \footnotesize \textit{GCS-Bézier tối ưu hình học thuần không phản ánh được hành vi này.}
        \end{column}
    \end{columns}
\end{frame}
