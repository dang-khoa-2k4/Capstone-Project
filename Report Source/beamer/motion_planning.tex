...
\begin{frame}{Nới lỏng lồi, Branch-and-Bound và Làm tròn}
    \small

    \textbf{Convex relaxation:}
    \begin{itemize}
        \item Nới $y_e\in\{0,1\}$ thành $y_e\in[0,1]$; dùng phối cảnh/RLT để giữ cấu trúc
              lồi~\cite{marcucci2023motion,marcucci2024shortest}.
        \item Giải bài toán tối ưu lồi $\Rightarrow$ nghiệm relaxation là cận dưới: $C_{\mathrm{relax}}\le C_{\mathrm{opt}}$.
    \end{itemize}

    \textbf{Xử lý nghiệm phân số:}
    \begin{itemize}
        \item \textbf{Branch-and-Bound:} tách nhánh trên biến $y_e$ phân số;
              cập nhật nghiệm khả thi tốt nhất từ relaxation $\Rightarrow$ cận trên và cận dưới khép lại $\Rightarrow$ giá trị tối ưu
              $C_{\mathrm{opt}}$ trong dung sai cho trước.
              % \item \textbf{Branch-and-Bound:} tách nhánh trên biến $y_e$ phân số;
              %       cập nhật nghiệm khả thi tốt nhất hiện tại và cận dưới từ relaxation.
              %       Khi cận trên và cận dưới khép lại $\Rightarrow$ giá trị tối ưu
              %       $C_{\mathrm{opt}}$ trong dung sai cho trước.

        \item \textbf{Randomized rounding:} xem $y_e^\star$ như trọng số xác suất,
              tìm đường đi bằng backtracking $\Rightarrow$ cố định path và giải lại
              bài toán lồi nhỏ.
    \end{itemize}

    \[
        C_{\mathrm{relax}}\le C_{\mathrm{opt}}\le C_{\mathrm{round}},
        \qquad
        \delta_{\mathrm{opt}}
        \le
        \delta_{\mathrm{relax}}
    ...
    \]
    \vspace{-0.5em}
    {\scriptsize $C_{\mathrm{opt}}$ chỉ biết chính xác khi BB bảo đảm tối ưu toàn cục hoặc khi $C_{\mathrm{relax}}=C_{\mathrm{round}}$.}
\end{frame}
