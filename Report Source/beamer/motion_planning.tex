    ...
    \begin{columns}[T, onlytextwidth]
        \begin{column}{0.43\textwidth}
            ...
            \begin{itemize}
                ...
                \item $z_{uv}\!\in\!Q_u\!\cap\! Q_v$,\; $s_u^+ = s_v^- = z_{uv}$
            \end{itemize}

            % \vspace{0.2cm}
            \[
                J_{\mathrm{total}} = \sum_{v\in V_r} p_v\, J_v
            \]
        \end{column}

        \begin{column}{0.02\textwidth}
            \centering \rule{0.4pt}{0.5\textheight}
        \end{column}
        ...
        \begin{column}{0.55\textwidth}
            ...
            \[
                ...
                \int_0^{T_v}
                \!\Bigl(
                \underbrace{a}_{\text{thời lượng}}
                +
                \underbrace{b\,|\dot{q}_v(t)|^2}_{\text{vận tốc hình học}}
                +
                \underbrace{c\,|u_v(t)|^2}_{\text{năng lượng điều khiển}}
                \Bigr)\,dt
            \]
            ...
            \footnotesize
            $q_v(t):=\pi(x_v(t))$;\quad
            $\dot{q}_v = dq_v/dt$;\quad
            $T_v$ — thời lượng đoạn $v$
        \end{column}
    \end{columns}
...
% ===================== Slide DMS-2: Multiple Shooting — biến, IVP, defect =====================
\begin{frame}{DMS tham số hóa đoạn quỹ đạo, buộc tính liên tục qua defect constraint}
    ...
    \[
        s_v^-
        \;\xrightarrow{\;\mathrm{IVP}_v\;}
        x_v(1)
        \;\overset{\mathrm{defect}\,=\,0}{=}\;
        s_v^+
        \;=\;
        s_{v+1}^-
        \;\xrightarrow{\;\mathrm{IVP}_{v+1}\;}
        \cdots
    \]
    ...
    \begin{columns}[T, onlytextwidth]
        \begin{column}{0.50\textwidth}
            \textbf{Biến quyết định — vùng $v$:}
            \begin{itemize}
                \item $s_v^- \in \mathbb{R}^{n_x}$ — điểm bắn
                \item $s_v^+ \in \mathbb{R}^{n_x}$ — trạng thái ra (tự do)
                \item $\Delta_v \ge 0$ — thời lượng
                \item $u_v(\cdot)\in\mathscr{U}_v$, với
            \end{itemize}

            \[
                \mathscr{U}_v := \left\{
                    \tau \mapsto \mathcal{U}_v(\tau; w_v)
                    \mid
                    w_v \in \mathbb{R}^{n_w}
                \right\}
            \]
        ...
        \begin{column}{0.46\textwidth}
            \begin{alertblock}{Defect constraint}
                \[
...
% =====================================================================
\begin{frame}{TN4 - GCS-DMS: mê cung $15\!\times\!15$ (4 mẫu đại diện)}
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.4\textwidth}
            ...
            \begin{itemize}\itemsep0pt
                \item Defect $\sim 10^{-10}$: sai số động lực học nội đoạn
                \item Gap nối $\sim 10^{-9}$: sai lệch giữa hai đoạn liên tiếp
                \item Vi phạm an toàn $\sim 10^{-8}$: mức vượt ràng buộc tránh vật cản
            \end{itemize}
...
% =====================================================================
\begin{frame}{TN4 - Hồ sơ vận tốc \& dấu vết động lực phi tuyến}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.6\textwidth}
            \centering
            \begin{minipage}{0.48\linewidth}
